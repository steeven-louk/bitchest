...
const [form] = Form.useForm();
  ...
  {getUser?.map((item)=>(
    <tr key={item.id} className=' align-baseline'>
      ...
      <td className='border px-4 py-2 text-center font-bold'>{item.status}</td>
      <td className='border px-4 py-2 text-center font-bold'>
        <span onClick={showModal} className='text-[#fff] bg-blue-700 cursor-pointer mx-3 px-2 rounded-md'>update</span>
        <span className='text-[#fff] bg-red-700 cursor-pointer px-2 rounded-md' onClick={(e)=>deleteUser(item.id, e)}>delete</span>
      </td>
    </tr>
  ))}
